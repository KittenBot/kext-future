...
class RFID:
    ...
    def readBlock(self,blockAddr,wait=3):
                ...
                if status == self.MI_OK:
                    ...
                    if status == self.MI_OK:
                        ...
                        if (status == self.MI_OK):
                            ...
                            # Read data from card
                            (status, backData, backBits) = self.read(
                                blockAddr)
                            if (status == self.MI_OK):
                                hex_list = backData
                                asc_str=''
    ...
    def writeBlock(self,blockAddr,data,wait=3):
        blockAddr = AVAILABLEBLOCK[blockAddr]
        self.MRFC522_init()
        # Scan for cards
        dataASCLL = []
                    ...
                    if status == self.MI_OK:
                        ...
                        if (status == self.MI_OK):
                            ...
                            if (status == self.MI_OK):
                                ...
                                (status, backData, backBits) = self.write(
                                    blockAddr,
                                    dataASCLL)
                                if (status == self.MI_OK):
